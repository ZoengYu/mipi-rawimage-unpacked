# you can use this tool to check (3024,4032) and (6048,8064) image of 10 bit raw image
# this script is used to calculate the ROI of "GRGB Bayer" image, you can use to check ROI value and save it.
# this templete is able to choose 25.1/100.3 MB size raw file
## author : chengyuw
...
def img_save_window(cut_img, width, height):  # call ROI Window
    ...
    # print("screen_width={0},screen_height={1}".format(screen_width,screen_height))
    win.title("ROI_IMG")
    lb = tk.Label(bg="black", fg="white",
                  text="do you want to save ROI image?").pack()
    btn = tk.Button(text="yes", bg="white", command=save_image).pack()
    btn2 = tk.Button(text="close", bg="white", command=close_window).pack()
    # cv2 default is BGR, need to convert it to RGB
    cut_img_rgb = cv2.cvtColor(cut_img, cv2.COLOR_BGR2RGB)
...
def selectPath():
    ...
    file_path = tk.filedialog.askopenfilename(
        title='Select .raw file', filetypes=[('RAW', '*.raw')])
    root.destroy()
    return file_path
